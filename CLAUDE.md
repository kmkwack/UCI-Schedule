...
### Session 58 (DropdownPicker — separate backdrop fade from sheet slide)
- **`src/screens/SettingsScreen.tsx`** — `DropdownPicker` now uses `animationType="none"` with two independent `Animated.Value`s: `backdropAnim` (opacity 0→1 on open, 1→0 on close, 220ms timing) and `sheetAnim` (translateY 500→0 spring on open, 0→500 timing on close). Backdrop is an `Animated.View` with `StyleSheet.absoluteFill` so it appears/disappears without moving. Sheet slides up/down independently. Selecting an option calls `closePicker()` for the fade-out animation before unmounting. Added `StyleSheet` to RN imports.

### Session 58 (Fix: auth screens disappear on "Continue with Google")
- **`App.tsx`** — Added `userIdRef` (kept in sync with `userId` state). The `onAuthStateChange` SIGNED_OUT handler now guards on `userIdRef.current !== null`, so it only resets app state when the user was actually logged in. Previously, `handleGoogleSignIn` called `supabase.auth.signOut()` before starting OAuth (to clear any stale session), which fired SIGNED_OUT and wiped the auth stack back to Welcome.

### Session 58 (Fix: Invalid Refresh Token on startup)
- **`App.tsx`** — Replaced the `getSession()` startup check (which only reads AsyncStorage without validating against the server) with a `supabase.auth.onAuthStateChange` subscription. When Supabase's background auto-refresh fires and gets an invalid token, it internally fires `SIGNED_OUT`. The listener catches this and resets all app state (userId, timetables, authStack, etc.) so the user is returned to the Welcome screen instead of seeing a broken session. Returns the unsubscribe function for cleanup.
...
### Session 58 (AuthNavigator — slide-in/out animations fixed)
- **`App.tsx`** — Shared `slideAnim` approach kept, but timing fixed: `useLayoutEffect` (fires before native paint) calls `slideAnim.setValue(W)` so the entering screen is already off-screen before the first frame is drawn. `useNativeDriver: false` used for all AuthNavigator animations so the JS-side `setValue` takes effect immediately with no native-driver sync issues. `wasPushRef` tracks push vs pop so the `useEffect` spring only fires on push. `renderScreen` signature extended to `(s, goBack)` so back buttons receive the animated `goBack` instead of raw `popAuth`.

### Session 58 (ProfileDropdownPicker — separate backdrop fade from sheet slide)
- **`src/components/ProfileEditorScreen.tsx`** — Added `Animated` and `StyleSheet` to RN imports. `ProfileDropdownPicker` now uses `animationType="none"` with two `Animated.Value`s: `backdropAnim` (opacity 0→1 fade, 220ms) and `sheetAnim` (translateY 500→0 spring on open, 0→500 timing on close). Backdrop is `Animated.View` with `StyleSheet.absoluteFill` so it covers the screen without sliding. Sheet slides independently. Applies to Year, Major, and Gender pickers.

### Session 59 (Settings screen — pageSheet modal)
- **`App.tsx`** — Added `Modal` to RN imports. Replaced the absolute-positioned `View` wrapper for `SettingsScreen` with a `<Modal presentationStyle="pageSheet" animationType="slide">`. iOS now handles the sheet presentation (card appearance, swipe-to-dismiss, background scaling) natively.
- **`src/screens/SettingsScreen.tsx`** — Changed main header `paddingTop` from `insets.top + 12` to fixed `20` since inside a `pageSheet` modal `insets.top` returns 0.
...
### Session 60 (Dynamic boards — Supabase-backed board list + admin Create Board UI)
- **`src/screens/BoardScreen.tsx`** — Renamed `BOARDS` constant to `FALLBACK_BOARDS`. Added `boards` state initialized from fallback. Added `fetchBoards()`: queries Supabase `boards` table filtered by `school`, ordered by `created_at`; maps rows to `Board` type; only replaces state when rows exist (so hardcoded boards still show while table is empty). `fetchBoards()` is called on mount (alongside `fetchPosts`) and inside `handleRefresh`. All `BOARDS.*` references replaced with `boards.*`. Default board ID in `openNewPost`/`resetComposer` updated from hard-coded `'general'` to `boards[0]?.id ?? ''`. `openPostFromBoardList` fallback changed from `id === 'general'` to `category === null`.
- **`src/screens/SettingsScreen.tsx`** — Added `BOARD_ICON_OPTIONS` constant (24 Ionicons with paired colors/backgrounds). Added `CreateBoardView` component: shows a live preview card, editable Name + Description fields, a 4-column icon picker grid, and a "Create Board" button that INSERTs into the `boards` table and marks the request as `resolved`. Added `createBoardFor` state to `BoardRequestsScreen`; each request card now has a "Create Board from this Request" button that opens `CreateBoardView` pre-filled with the requester's name and description. Admin can edit both fields and pick any icon before applying.
- **Supabase SQL required** — See instructions below.

### Session 60 (Admin — Manage Active Boards screen with delete)
- **`src/screens/SettingsScreen.tsx`** — Added `ManageBoardsView` component: fetches all boards from Supabase for 'UC Irvine', renders each as a card with icon/name/description and a red trash button. Tapping trash shows a confirmation Alert then DELETEs the row from the `boards` table and removes it from local state. Added `showManageBoards` state to `BoardRequestsScreen`; a "Manage Active Boards" entry button now appears at the top of the Board Requests screen (above the requests list). Also added `DELETE` RLS policy required (see below).
- **Supabase SQL required** — `CREATE POLICY "Authenticated users can delete boards" ON boards FOR DELETE USING (auth.role() = 'authenticated');`
