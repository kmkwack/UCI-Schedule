          ...
          {rows.map((row) => (
            <View key={row.code} style={{ flexDirection: 'row', alignItems: 'center', gap: 9, opacity: row.dimmed ? 0.42 : 1 }}>
              ...
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text numberOfLines={1} ellipsizeMode="tail" style={{ fontSize: 14, fontWeight: '900', color: row.dimmed ? '#aeb7ca' : '#16203a' }}>{row.code}</Text>
                <Text numberOfLines={1} ellipsizeMode="tail" style={{ fontSize: 12, color: '#7a859c', marginTop: 1 }}>{row.title}</Text>
              </View>
            </View>
          ))}
        </View>
      ...
      {rows.map(([time, code, room, color]) => (
        <View key={code} style={{ flexDirection: 'row', alignItems: 'stretch', gap: 12 }}>
          ...
          <View style={{ flex: 1, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, backgroundColor: `${color}14`, borderLeftWidth: 3, borderLeftColor: color }}>
            <Text numberOfLines={1} ellipsizeMode="tail" style={{ fontSize: 13, fontWeight: '800', color: PALETTE.ink, marginBottom: 2 }}>{code}</Text>
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text numberOfLines={1} ellipsizeMode="tail" style={{ fontSize: 12, lineHeight: 17, color: isDark ? 'rgba(255,255,255,0.58)' : PALETTE.inkMuted }}>
